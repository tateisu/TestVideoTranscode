...
class ActMain : AppCompatActivity() {
    ...
    class VM(
        ...
        val arInputPicker = ActivityResultHandler { result ->
            result.takeIf { it.resultCode == RESULT_OK }
                ?.data?.data?.let { handleInputUri(it) }
        }
        ...
        fun handleInputUri(uri: Uri) = viewModelScope.launch {
            try {
                val context = app
                logText.value = "処理開始"
                ...
                withContext(Dispatchers.IO) {
                    ...
                    try {
                        ...
                        withContext(Dispatchers.Main) {
                            ...
                            logText.value = "変換終了 $result"
                            compressedPath.value = result.file.canonicalPath
                            compressedMimeType.value = result.mimeType
                        }
                    } finally {
        ...
        fun openOutput(activity: Activity) {
            try {
                ...
                Log.e(TAG, "openOutput failed", ex)
            }
        }
    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        views.run {
            ...
            btnChooseInput.setOnClickListener { vm.openPicker() }
